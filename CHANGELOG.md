...
- **3 מתוך 5 פריטים בהקשר כסף** בכל סשן של בת 7–8, בתוך המאגרים
  ...
  ("3 מתוך 5 תרגילים בהקשר כסף — שילוב חינוך פיננסי") ו-§5.4
  ("חינוך פיננסי מבוזר על פני מתמטיקה, לא נושא נפרד"). לא מיומנות
  חדשה — ערבוב הקשר בלבד.
- **טיפוסים:** `AddSubItem` ו-`MultItem` קיבלו שני שדות אופציונליים:
  `context?: "money"` ו-`explanation?: string`. פריטים יבשים נשארו
  ללא שינוי.
- **מאגרים:** 30 פריטי בעיות-כסף נוספו ל-`add-sub-100.json` ו-30
  ל-`multiplication.json` — 6 לכל דרגה (1–5), עברית RTL, ₪ אחרי
  המספר, הסבר embedded לכל פריט (CPA-מותאם בהקשר כסף).
- **לוגיקת בחירה:** `selectNextItem` קיבל פרמטר אופציונלי
  `desiredContext?: "money" | "plain"`. מסנן את ה-pool, ועם fallback
  בטוח ל-pool המלא אם ה-pool המסונן ריק (לא תוקע סשן).
  הפונקציה `nextDesiredContext(money, plain)` מציעה את ההקשר
  הבא לפי חלון 3-מ-5 (3 כסף, 2 יבש).
- **דף סשן:** ה-state עוקב אחרי המונים `moneyShownRef` ו-`plainShownRef`
  ומתאפס כל 5 פריטים. בחירת ההקשר מופעלת רק לבת 7–8 ב-`add_sub_100`
  או `multiplication`. שאר הגילים והמיומנויות לא מושפעים.
- **רינדור:** `ItemPrompt` של פריט money מוצג כמשפט עברי RTL בפונט
  בינוני (`text-xl md:text-2xl`) במקום סימון מתמטי גדול. `ItemReveal`
  מציג את ה-`explanation` של הפריט עצמו במקום `explain.ts`.
- **בדיקות:** `tests/unit/money-context-bank.test.ts` חדש, 16 מקרים
  על שני המאגרים: גודל, פיזור דרגות, ₪, הסבר, נכונות חישוב, טווח
  תשובה, סריקת ביטויים אסורים, גיוון דמויות (לא יותר מ-3 פריטים
  שחולקים שם דמות). `tests/unit/adaptive.test.ts` הורחב ב-9 מקרים
  על סינון הקשר ועל `nextDesiredContext`. `tests/unit/multiplication.test.ts`
  התעדכן: regex של "%d × %d = ?" חל רק על פריטים יבשים.
  סה"כ: **306 → 322 עוברות**.
- **תיעוד:** `docs/parent-guide.md §4` עודכן עם הסבר על ערבוב
  3-מ-5 בעיות-כסף לבת 7–8.

### Changed — תרגילים בסשן לפי גיל (תיקון drift מ-`.env.local`)
- **`itemsPerSessionForAge(age)` חדש** ב-`src/lib/profiles.ts`: גיל 7–8 → 15
  תרגילים, גיל 9–10 → 18, אחרת 10. מתאם ל-MyLevel §3.1 (10–12 דק' לבת 7,
  15 דק' לבת 9), עם `+3` כמרווח שבחרה Marina (2026-04-27).
- **`src/app/session/page.tsx`** מחשב כעת `itemsPerSession` per-profile
  לפי גיל. הקבוע הגלובלי `ITEMS_PER_SESSION` הוסר.
- **`NEXT_PUBLIC_ITEMS_PER_SESSION` הופך ל-dev-override:** כשהוא מוגדר, דורס
  את ערכי הגיל. בפרודקשן להשאיר ריק. `.env.example` מתועד בהתאם.
- **למה:** Marina עברה על מצב הפרויקט וגילתה ש-`.env.local` שלה הגדיר 15
  גלובלית מבלי שנעשתה החלטה מודעת על הערך. הקבוע הגלובלי לא תאם את
  המודל הפדגוגי של MyLevel (גילים שונים = זמני סשן שונים). עכשיו הקוד
  עוקב אחרי המסמך, לא אחרי דריסה ב-`.env.local`.
- **בדיקות:** 5 מקרים חדשים ב-`tests/unit/profiles.test.ts` (גילים
  7/8/9/10/חוץ-טווח). סה"כ: **292 → 297**.
- **תיעוד:** `docs/parent-guide.md` §4 עודכן עם טבלת תרגילים-לפי-גיל
  ודוגמה חדשה ל-graduation לאוולין (15/סשן, ≤2 סשנים במקום 3–4).
- **משתמשת:** Marina צריכה למחוק/להעיר את השורה
  `NEXT_PUBLIC_ITEMS_PER_SESSION=15` ב-`.env.local` שלה כדי שערכי הגיל
  ייכנסו לפועל. אם תשאיר אותה — ההתנהגות זהה לקודם (15 לכולן).
