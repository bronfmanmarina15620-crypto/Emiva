# תוכנית — MATH-BAT9-002 (ops_1000, סלייס 2 של אמיליה)

## מטרה
להוסיף לאמיליה מיומנות שנייה (חיבור/חיסור עד 1000) עם routing אוטומטי אחרי
graduation מ-`fractions_intro`. שימוש חוזר מלא של לולאת Model A, של ItemInput,
...
2. **טווח:** תוצאות ב-[0, 999]. בלי תוצאות שליליות (לא נפוץ בגיל 9 בלי
...
4. **Explain** — הקיים מספיק. פירוק ל-tens + ones כבר מחזיק; פירוק ל-hundreds
   לא קריטי בגיל 9 כשהכמות קטנה ומזוהה. אם אמיליה מתקשה בהסברים — backlog.
...
**סדר מכוון:** fractions ראשון כי הוא הסלייס הפדגוגי הראשון של אמיליה
...
### 6. מאגר — ≥30 פריטים, ~50/50 add/sub
...
`explain` כבר אוניברסלי ל-`AddSubItem`. מעבירים פריטים של ops_1000 — עובד.
...
7. Docs: ROADMAP (Done), CHANGELOG (Unreleased), parent-guide §4 (יעד routing).
8. ולידציה.
...
| אמיליה לא מבינה למה פתאום מספרים גדולים | באנר graduation שהופיע בסיום fractions אומר "פרק חדש בדרך" — משמש כ-onboarding implicit |
| מאגר הפריטים מתקלקל אחרי copy-paste (answer ≠ operands) | Test computed: לכל פריט, `answer === operands[0] op operands[1]` |
| פערים בין דרגות — למשל T3 וגם T4 עם carry יחיד | Criterion מפורש: T3=single carry, T4=multiple. Test מוודא. |
