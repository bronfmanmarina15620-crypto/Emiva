# Plan — MATH-EVELYN-MULT-001

## מטרה
Skill שני ל-Evelyn: לוחות הכפל 2–10. routing אוטומטי אחרי graduation מ-
`add_sub_100`. reveal מבוסס CPA (קבוצות, ספירת-קפיצה, עוגני ×10/×5/×9).

## החלטות פדגוגיות נעולות

1. **טווח:** לוחות 2–10 בלבד. ללא ×1 (טריוויאלי), ללא ×11/×12 (לא סטנדרטי
   בישראל).
2. **שיטת ה-reveal לכל table:**
...
3. **Commutativity:** tier 5 מערבב (`7 × 8` ו-`8 × 7`) כדי שהילדה תראה את
   החוק — לא לומדים אותו explicitly ב-slice הזה.
...
`isItemCorrect` מקבלים אותו אוטומטית דרך אותה ענף.
...
אמורה לשלוט בו ממילא. לכן לא מוסיפים ל-9–10 בסלייס הזה. אם Emilia מתקשה
— backlog item.

### 5. Item bank — 30 פריטים
- T1 (anchors: ×2, ×5, ×10): 6 פריטים (2 מכל table)
- T2 (×3, ×4): 6 פריטים (3 מכל table)
- T3 (×6, ×9): 6 פריטים (3 מכל table)
- T4 (×7, ×8): 6 פריטים (3 מכל table)
...
| Bank של 30 פריטים לא מכסה את כל הטבלאות | Test מוודא שכל table 2..10 מופיע ≥ 1 פעם |
| Legacy profiles עם `allowedSkills: ["add_sub_100"]` בלבד | `loadProfiles` מחזיר נגזרת מגיל בכל טעינה — אוטומטי |
