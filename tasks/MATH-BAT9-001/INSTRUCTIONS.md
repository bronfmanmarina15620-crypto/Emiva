...
- **חשיפה מבוססת-שיטה** לפי CLAUDE.md §כלל UX של תרגול — בניסיון 3,
  מציגים את הפס המלא + הסבר חד-שורה: "בחצי יש חלק אחד מתוך שניים שווים".
  קצר, פיקטוריאלי-תחילה.

### טסטים
לפי `CLAUDE.md §סטנדרטים הנדסיים` — טסטים לצד קוד:
...
- **הצגת pie-chart** — פס אופקי בלבד לסלייס הזה; pie
  יכולה להגיע כשיהיו ≥ 2 סיבות קונקרטיות (CLAUDE.md §סטנדרטים הנדסיים).
...
- `CLAUDE.md §כלל UX של תרגול` — לולאת 3 ניסיונות, חשיפה מבוססת-שיטה.
- `CLAUDE.md §טון — רק growth-mindset` — ביטויים אסורים, מסגור נדרש.
