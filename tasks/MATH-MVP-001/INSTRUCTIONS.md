...
- title: Emiva Math MVP — לולאת ליבה, בת 7, חיבור/חיסור עד 100
- owner: Marina (PM + CTO)
- priority: P0
- target_branch: feat/math-mvp-001
- due_date: TBD
- references:
  - `CLAUDE.md` (חוזה הריפו, stack, פורמט תגובה)
  - `MyLevel.docx` §3.1 (מתמטיקת ליבה, בת 7), §11 (מדידה והחלטות)
  - `progressive_schools_research.docx` (Model A — לא לדלג)
  - `master_curriculum.docx` (רציונל תכנית הלימודים)
...
- **לולאת 3 ניסיונות לכל item** (עדכון 2026-04-19):
  ...
  - **שליטה:** רק ניסיון 1 נכון סופר כ"נכון" לצורך mastery %. ניסיון
...
- בעיות מילוליות בהקשר כסף → טאסק נפרד (MATH-MONEY-001).
- ויזואליזציות Concrete→Pictorial→Abstract (סינגפור) → טאסק נפרד.
...
- Audio / אנימציות / gamification מעבר לפידבק בסיסי.
...
3. מאגר פריטים: `add-sub-100.json` ≥ 30 items, מאוזן ב-5 רמות קושי.
4. Unit tests ירוקים:
...
- **ידני — golden path:** לפתוח `localhost:3000/session` ב-Chromium,
...
3. להישאר בתוך טווח. לא להוסיף מיומנויות, תוכן, או אנימציות.
4. **שני מקרים קונקרטיים לפני הפשטה** (כלל CLAUDE.md). לא לבנות
   `GenericScheduler<T>` עבור skill אחד.
5. אם מתגלית ארכיטקטורה עדיפה שמשנה טווח — **לעצור ולהסביר** לפני
   המשך.
...
- [ ] המטרה הושגה: בת 7 מבצעת סשן 10-item end-to-end בדפדפן.
- [ ] הטווח נשמר — לא נוספה מיומנות/שפה/תוכן מחוץ לטווח.
- [ ] כל התוצרים הופקו (1–7).
- [ ] כל ולידציה רצה והתוצאות דווחו.
- [ ] סיכונים שנותרו מתועדים ב-PR description.
- [ ] הדוח הסופי כולל: קבצים שהשתנו, סטטוס ולידציה, פריטים פתוחים.
...
- **מבחן חיצוני:** מחוץ לטווח של הטאסק (→ MATH-EXTERNAL-TEST-001).
...
| סיכון | מיטיגציה |
|-------|----------|
| מאגר פריטים קטן מדי → adaptive לא נצפה | ≥ 30 items, 5 רמות קושי, ≥ 5 בכל רמה |
...
| זחילת טווח לבת 9 / בעיות מילוליות | לסרב בתוך הסשן; לפתוח טאסק חדש |

## Handoff

- **PM → Eng:** אישור טווח + אילוצים לפני פתיחת plan.
